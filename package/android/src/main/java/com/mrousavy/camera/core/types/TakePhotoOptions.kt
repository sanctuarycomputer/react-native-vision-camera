...
data class TakePhotoOptions(val file: OutputFile, val flash: Flash, val enableShutterSound: Boolean, val useFastMode: Boolean, val resolveOnCaptureStarted: Boolean) {
  ...
  companion object {
    fun fromJS(context: Context, map: ReadableMap): TakePhotoOptions {
      ...
      val resolveOnCaptureStarted = if (map.hasKey("resolveOnCaptureStarted")) map.getBoolean("resolveOnCaptureStarted") else false

      val outputFile = OutputFile(context, directory, ".jpg")
      return TakePhotoOptions(outputFile, flash, enableShutterSound, useFastMode, resolveOnCaptureStarted)
    }
  }
}
